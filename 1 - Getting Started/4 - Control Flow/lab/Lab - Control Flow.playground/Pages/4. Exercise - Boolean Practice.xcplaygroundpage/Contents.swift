...
if hasFish, hasPizza, hasVegan == true {
    print("Let's go!")
} else { //because all of the cases are not proven true, the else statement is run 
...
let isNiceWeather = temp >= 82 && isSunny //can state what the constant will be evaluated as 
if isNiceWeather {
    print("Time to go on a walk!")
} else {
