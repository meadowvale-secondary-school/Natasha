...
struct Rectangle {
    var width: Int
    var height: Int
    var area: Int { //to add a computed property: 1) must declare property as a variable
        // 2) use curly brackets to define the logic that calculates the value to return
        return width * height
    }
}

let rectangleArea = Rectangle(width: 20, height: 20)
print(rectangleArea.area)//3) you can access computed properties using dot syntax, just like with another property

// logic contained will be executed each time the property is accessed, so the returned value will always be up to date
/*:
...
var myHeight2 = Height(heightInCentimeters: 155)
print(myHeight2.heightInInches)

var myHeight = Height(heightInInches: 61.02)
