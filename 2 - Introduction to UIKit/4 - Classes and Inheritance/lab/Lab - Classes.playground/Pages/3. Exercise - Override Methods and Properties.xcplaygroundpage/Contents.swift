...
class ShieldedShip: Fighter {
    ...
    override func wasHit() { //overides original if/else condition of wasHit
        if shieldStrength > 0 {
            shieldStrength -= 5
        } else {
            super.wasHit() //calls back to superclass implementation of wasHit() instead of overriding this section of the condition
        }
    }
}

var defender = ShieldedShip(name: "Defender", weapon: "Cannon")

defender.moveRight()
print(defender.position) //moves 1 position right

defender.fire()
print(defender.remainingFirePower) //removes a firepower, and prints 4
 ...
 */
defender.wasHit() //uses override if/else condition creating new situation
